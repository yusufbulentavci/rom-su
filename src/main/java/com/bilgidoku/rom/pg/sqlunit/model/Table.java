...
public class Table extends TypeComp implements CGType {
	...
	public boolean isInheritsProsedur() {
		if (inheritType == null)
			return false;
		...
		if (inheritType.schema.equals("sistem") && inheritType.table.equals("prosedur"))
			return true;
...
//	}
	...
	@Override
	public void writeValue(DbThree db3, JSONWriter jsonWriter) throws KnownError, KnownError {
		jsonWriter.object();
		for (Field field : getAllFields()) {
			jsonWriter.key(field.name);
			field.getTypeHolder().writeJson(jsonWriter, db3);
