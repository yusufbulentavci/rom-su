...
public class FunctionParser extends CompParser {
	...
	public SuComp parse(SqlUnit su, String trimmedLine, String nextLine, int lineNo) throws SuException {
		...
		boolean noRom = (trimmedLine.indexOf(" norom") > 0);
		Function em = new Function(su, matchFunc.group(1), matchFunc.group(2), unitTest, lineNo, noRom);
		...
		for (String string : methodTokens) {
			...
			}else if (string.startsWith("dokun=")) {
				String hm = string.substring("dokun=".length());
				em.setDokun(hm.trim());
